...
export function FeaturesContent({ initialModule }: Props) {
    ...
    const handleSelect = (idx: number) => {
        setActive(idx);
        // Scroll vers le contenu du module avec offset pour la nav + tabs sticky
        if (moduleRef.current) {
            const offset = 68 + 64; // nav + tabs sticky
            const top = moduleRef.current.getBoundingClientRect().top + window.scrollY - offset;
    ...
    return (
        <>
                    ...
                    <div className="max-w-3xl">
                        ...
                        <RevealOnScroll delay={0.1}>
                            <p className="mt-6 text-lg lg:text-xl max-w-[56ch]">
                                Sept modules pensés pour s&apos;assembler. Activez ce dont vous avez
                                besoin, étendez progressivement, payez à l&apos;usage réel.
                            </p>
                        </RevealOnScroll>
            ...
            >
                <Container>
                    <div className="flex gap-1 overflow-x-auto no-scrollbar py-3" role="tablist">
                        {MODULES.map((mod, i) => {
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.15fr] gap-10 lg:gap-14 items-start mb-14">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-3 gap-4 mb-8">
                                    {m.kpis.map((k) => (
                                        <div key={k.label} className="border-l-2 pl-3" style={{ borderColor: m.color }}>
                                            <div
                                ...
                                <RevealOnScroll key={f.title} delay={(i % 3) * 0.08}>
                                    <div className="h-full bg-white border border-border-default rounded-card p-6">
                                        <div
                                            ...
                                            style={{ background: `${m.color}15` }}
                                        >
                                            <Check size={16} style={{ color: m.color }} strokeWidth={3} />
                                        </div>
